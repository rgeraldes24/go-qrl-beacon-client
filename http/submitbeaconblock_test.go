...
func TestSubmitBeaconBlock(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Good",
			randaoReveal: zond.MLDSA87Signature([4627]byte{
			...
			slot:                   zond.Slot(uint64(time.Since(genesisResponse.Data.GenesisTime).Seconds())/uint64(slotDuration.Seconds())) + 1,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if res.Data.Version != spec.DataVersionZond {
				t.Skip("proposal version not supported by endpoint, skipping test")
			}

			signedBeaconBlock := &spec.VersionedSignedBeaconBlock{}
			switch {
			case res.Data.Zond != nil:
				signedBeaconBlock.Version = spec.DataVersionZond
				signedBeaconBlock.Zond = &zond.SignedBeaconBlock{
					Message:   res.Data.Zond,
					Signature: zond.MLDSA87Signature([4627]byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}),
				}
			default:
