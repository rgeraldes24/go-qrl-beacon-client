...
type blindedBeaconBlockBodyJSON struct {
	...
	ExecutionData          *zond.ExecutionData          `json:"execution_data"`
	Graffiti               string                       `json:"graffiti"`
	ProposerSlashings      []*zond.ProposerSlashing     `json:"proposer_slashings"`
	AttesterSlashings      []*zond.AttesterSlashing     `json:"attester_slashings"`
	Attestations           []*zond.Attestation          `json:"attestations"`
	Deposits               []*zond.Deposit              `json:"deposits"`
	VoluntaryExits         []*zond.SignedVoluntaryExit  `json:"voluntary_exits"`
	SyncAggregate          *zond.SyncAggregate          `json:"sync_aggregate"`
	ExecutionPayloadHeader *zond.ExecutionPayloadHeader `json:"execution_payload_header"`
}
...
func (b *BlindedBeaconBlockBody) unpack(data *blindedBeaconBlockBodyJSON) error {
	...
	if len(randaoReveal) != zond.SignatureLength {
	...
	if len(graffiti) != zond.GraffitiLength {
