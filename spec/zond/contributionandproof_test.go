...
func TestContributionAndProofJSON(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:  "JSONBad",
			input: []byte("[]"),
			err:   "invalid JSON: json: cannot unmarshal array into Go value of type zond.contributionAndProofJSON",
		},
		{
		...
		},
		{
			...
			err:   "invalid JSON: invalid JSON: json: cannot unmarshal bool into Go value of type zond.syncCommitteeContributionJSON",
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			var res zond.ContributionAndProof
